...
// Returns a phase coefficient based on total material:
// 1.0 = opening, 0.0 = endgame (continuous)
double compute_phase_coeff(int totalMaterial) {
    constexpr int OPENING_MATERIAL = 12000;
    constexpr int ENDGAME_MATERIAL = 3000;

    double coeff = double(totalMaterial - ENDGAME_MATERIAL) /
                   (OPENING_MATERIAL - ENDGAME_MATERIAL);

    return std::clamp(coeff, 0.0, 1.0);
}

// Adds new blending logic and shuffling functions
int blend_nnue_with_simple(int nnue, int simpleEval, int nnueComplexity, int materialImbalance) {
    int weight = std::clamp(100 - nnueComplexity / 2 - materialImbalance / 4, 50, 100);
...
int totalMaterial = simple_eval(pos, WHITE) + simple_eval(pos, BLACK);

// Compute continuous phase coefficient and update weights
double phase_coeff = compute_phase_coeff(totalMaterial);
NNUE::update_weights(phase_coeff);

    // Continue with the evaluation logic
