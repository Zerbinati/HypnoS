...
trace(Position& pos, const Eval::NNUE::Networks& networks, Eval::NNUE::AccumulatorCaches& caches) {
    ...
    int materialWeight   = 1024 - delta;
    int positionalWeight = 1024 + delta;

    if (Eval::useDynamicStrategy)
    {
        int phase = pos.count<ALL_PIECES>();
        materialWeight   -= (32 - phase) * 8;
    ...
    for (File f = FILE_A; f <= FILE_H; ++f)
        for (Rank r = RANK_1; r <= RANK_8; ++r)
        {
            ...
            if (pc != NO_PIECE && type_of(pc) != KING)
            {
                ...
                // Use local temp weights to avoid affecting outer context
                int tempMaterialWeight   = 1024 - delta;
                int tempPositionalWeight = 1024 + delta;
               ...
               tempPositionalWeight += Eval::PositionalEvaluationStrategy;
                ...
                Value eval = static_cast<Value>(
                    ((tempMaterialWeight + Eval::MaterialisticEvaluationStrategy) * psqt +
                     (tempPositionalWeight + Eval::PositionalEvaluationStrategy) * positional)
                    / (1024 * OutputScale));
