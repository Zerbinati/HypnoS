...
using namespace Hypnos::Eval::NNUE;
...
template<typename Arch, typename Transformer>
Hypnos::Eval::NNUE::NetworkOutput Hypnos::Eval::NNUE::Network<Arch, Transformer>::evaluate(
    const Hypnos::Position& pos, 
    Hypnos::Eval::NNUE::AccumulatorCaches::Cache<Hypnos::Eval::NNUE::Network<Arch, Transformer>::FTDimensions>* cache) const {
    ...
    // Utilizzo delle variabili globali MaterialisticEvaluationStrategy e PositionalEvaluationStrategy
    int materialisticValue = static_cast<Value>(psqt / OutputScale);
    int positionalValue = static_cast<Value>(positional / OutputScale);

    return std::make_tuple(materialisticValue, positionalValue);
}


template<typename Arch, typename Transformer>
void Hypnos::Eval::NNUE::Network<Arch, Transformer>::verify(std::string evalfilePath) const {
    if (evalfilePath.empty())
        evalfilePath = evalFile.defaultName;
