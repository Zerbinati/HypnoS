...
string engine_info(bool to_uci) {
    ...
    ss << "HypnoS Iccf " << Version << setfill('0');
    ...
    ss << "\n"
       << compiler_info() << "\nBuild date/time       : " << year << '-' << setw(2) << setfill('0')
       << month << '-' << setw(2) << setfill('0') << day << ' ' << __TIME__
...
void init_hw_info() {
    ...
    static GLPIEX impGetLogicalProcessorInformationEx = (GLPIEX) (void (*)(void)) GetProcAddress(
      GetModuleHandle("kernel32.dll"), "GetLogicalProcessorInformationEx");
        ...
        while (offset < len && offset + ptr->Size <= len)
        {
            if (ptr->Relationship == RelationNumaNode)
            {
                if (numaNodeCount == maxNodes)
                {
                    ...
                    nodeGroupMask =
                      (GROUP_AFFINITY*) realloc(nodeGroupMask, maxNodes * sizeof(GROUP_AFFINITY));
                    if (!nodeGroupMask)
                    {
                        release_memory();
                        return;
    ...
    while (std::getline(ss, line))
    {
        ...
        else if (regex_search(line, match, rgxL1dCache) || regex_search(line, match, rgxL1iCache))
        {
            processorCacheSize[0] +=
              (uint32_t) atoi(match[1].str().c_str()) * parse_unit(match[2].str().c_str());
        }
...
void init_processor_brand() {
    ...
    LONG  result =
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("Hardware\\Description\\System\\CentralProcessor\\0\\"),
                   0, KEY_READ, &hKey);
    if (result == ERROR_SUCCESS)
    {
        //Get value of 'Key = ProcessorNameString' which is the processor name
        result = RegQueryValueEx(hKey, TEXT("ProcessorNameString"), NULL, NULL, (LPBYTE) &Data,
                                 &buffersize);
...
void init_mem_info() {
    ...
    while (std::getline(memInfo, line))
    {
        ...
        if (regex_search(line, match, rgxMemTotal))
        {
            ...
            if (strcasecmp(match[2].str().c_str(), "KB") == 0
                || strcasecmp(match[2].str().c_str(), "KiB") == 0)
                totalMemory *= 1024;
            else if (strcasecmp(match[2].str().c_str(), "MB") == 0
                     || strcasecmp(match[2].str().c_str(), "MiB") == 0)
                totalMemory *= 1024 * 1024;
            else if (strcasecmp(match[2].str().c_str(), "GB") == 0
                     || strcasecmp(match[2].str().c_str(), "GiB") == 0)
