...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(9) + int(avg) * avg / 10502;
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1578, 1273);
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 13999
    ...
    probCutBeta = beta + 185 * improving + 150 * ((ss + 1)->cutoffCnt > 3);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                int   captHist =
                  thisThread->captureHistory[movedPiece][to_sq(move)][type_of(capturedPiece)];
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)]
                      ...
                      + (*contHist[1])[movedPiece][to_sq(move)] - 5169;
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && pos.non_pawn_material(us))
        {
                ...
                && type_of(move) != PROMOTION)
            {
                ...
                Value futilityValue = futilityBase + PieceValue[pos.piece_on(to_sq(move))];
