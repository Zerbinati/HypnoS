...
void apply_exploration(std::vector<Move>& moveList, int depth) {
    ...
    for (const auto& move : moveList) {
        int explorationBonus = rng.rand<int>() % (10 + depth); // Random bonus
        scoredMoves.push_back({move, explorationBonus});
    }

    std::sort(scoredMoves.begin(), scoredMoves.end(), [](const ScoredMove& a, const ScoredMove& b) {
        return a.score > b.score; // Sorts by score
    });

    moveList.clear();
    for (const auto& scoredMove : scoredMoves) {
        moveList.push_back(scoredMove.move);
    }
}
...
moves_loop:  // When in check, search starts here
    ...
    else if (bestMove)
    {
        ...
        if (!PvNode)
            ttMoveHistory << (bestMove == ttData.move ? 800 * (depth > 5) : -879);
    }
