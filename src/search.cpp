...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(9) + int(avg) * avg / 12804;
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 16620 && eval >= beta
        && ss->staticEval >= beta - 21 * depth + 330 && !excludedMove && pos.non_pawn_material(us)
    ...
    probCutBeta = beta + 181 - 68 * improving + 150 * ((ss + 1)->cutoffCnt > 3);
