...
Depth reduction(bool i, Depth d, int mn, int delta, int rootDelta) {
    ...
    return (reductionScale + 1123 - int(delta) * 832 / int(rootDelta)) / 1024
         + (!i && reductionScale > 1025);
}
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(11) + int(avg) * avg / 11254;
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 18001 && eval >= beta
        && ss->staticEval >= beta - 21 * depth + 315 && !excludedMove && pos.non_pawn_material(us)
