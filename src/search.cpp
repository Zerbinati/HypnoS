...
Depth reduction(bool i, Depth d, int mn, int delta, int rootDelta) {
    ...
    return (reductionScale + 1316 - int(delta) * 850 / int(rootDelta)) / 1024
...
int stat_bonus(Depth d) { return std::clamp(235 * d - 315, 0, 1291); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 563 * d - 290 : 1233); }
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            delta     = Value(11) + int(avg) * avg / 12350;
            alpha     = std::max(avg - delta, -VALUE_INFINITE);
            beta      = std::min(avg + delta, int(VALUE_INFINITE));

            // Adjust optimism based on root move's averageScore (~4 Elo)
            optimism[us]  = 129 * avg / (std::abs(avg) + 90);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1591, 1315);
    ...
    if (eval < alpha - 480 - (283 - 144 * ((ss + 1)->cutoffCnt > 3)) * depth * depth)
    {
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 16381 && eval >= beta
    ...
    probCutBeta = beta + 175 - 62 * improving + 155 * ((ss + 1)->cutoffCnt > 3);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + 292 + 281 * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4060 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][from_to(move)];

                lmrDepth += history / 5578;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 59 ? 131 : 58) + 128 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            if (!rootNode && move == ttMove && !excludedMove
                ...
                && tte->depth() >= depth - 3)
            {
                Value singularBeta  = ttValue - (60 + 58 * (ss->ttPv && !PvNode)) * depth / 64;
                ...
                if (value < singularBeta)
                {
                    ...
                    {
                        extension = 2 + (value < singularBeta - 17 && !ttCapture);
                        depth += depth < 14;
                    }
        ...
        if (depth >= 2 && moveCount > 1 + rootNode)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + 44 + 2 * newDepth);  // (~1 Elo)
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 14305 && value > -12167)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14873)
                  + ((ss - 1)->moveCount > 11)
                  + (!ss->inCheck && bestValue <= ss->staticEval - 147);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 232;
    }
