...
Depth reduction(bool i, Depth d, int mn, int delta, int rootDelta) {
    ...
    return (reductionScale + 1334 - int(delta) * 876 / int(rootDelta)) / 1024
...
int stat_bonus(Depth d) { return std::clamp(240 * d - 320, 0, 1293); }

// History and stats update malus, based on depth
int stat_malus(Depth d) { return (d < 4 ? 554 * d - 303 : 1208); }
...
void Thread::search() {
           ...
           && !(Limits.depth && mainThread && rootDepth > Limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
        {
            ...
            optimism[us]  = 130 * avg / (std::abs(avg) + 89);
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-13 * int((ss - 1)->staticEval + ss->staticEval), -1585, 1302);
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 16226 && eval >= beta
    ...
    probCutBeta = beta + 175 - 62 * improving + 150 * ((ss + 1)->cutoffCnt > 3);
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            else
            {
                ...
                if (lmrDepth < 6 && history < -4050 * depth)
                    continue;

                history += 2 * thisThread->mainHistory[us][from_to(move)];

                lmrDepth += history / 5645;

                Value futilityValue =
                  ss->staticEval + (bestValue < ss->staticEval - 59 ? 137 : 58) + 127 * lmrDepth;
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
                ...
                && tte->depth() >= depth - 3)
            {
                ...
                if (value < singularBeta)
                {
                    ...
                    {
                        extension = 2 + (value < singularBeta - 19 && !ttCapture);
                        depth += depth < 14;
                    }
        ...
        if (depth >= 2 && moveCount > 1 + rootNode)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                const bool doDeeperSearch    = value > (bestValue + 45 + 2 * newDepth);  // (~1 Elo)
        ...
        if (value > bestValue)
        {
            ...
            if (value > alpha)
            {
                ...
                else
                {
                    ...
                    if (depth > 2 && depth < 12 && beta < 14212 && value > -12082)
                        depth -= 2;
    ...
    {
        int bonus = (depth > 5) + (PvNode || cutNode) + ((ss - 1)->statScore < -14976)
                  + ((ss - 1)->moveCount > 11)
                  + (!ss->inCheck && bestValue <= ss->staticEval - 145);
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + 229;
    }
...
void update_all_stats(const Position& pos,
    ...
    {
        int bestMoveBonus = bestValue > beta + 171 ? quietMoveBonus      // larger bonus
