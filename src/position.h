...
class Position {
   public:
    ...
    // Functions to calculate bonuses/penalties based on style
    bool is_near_enemy_king(Square s) const;       // Proximity to the enemy king
    bool is_isolated(Square s, Bitboard pawns) const; // Isolated pawns
    bool has_bishop_pair(Color c) const;          // Presence of the bishop pair
    bool is_on_seventh_rank(Square s, Color c) const; // Rook on the seventh rank
