...
// Checks if a piece is near the enemy king (within two squares of distance)
bool Position::is_near_enemy_king(Square s) const {
    Square enemyKing = square<KING>(~sideToMove);
    return distance(s, enemyKing) <= 2;
}
...
bool Position::is_isolated(Square s, Bitboard pawns) const {
    File f = file_of(s);
    return !(pawns & adjacent_files_bb(f));
}

// Checks for the presence of a bishop pair for the given color
bool Position::has_bishop_pair(Color c) const {
    return popcount(pieces(c, BISHOP)) >= 2;
}

// Checks if a rook is on the seventh rank relative to the given color
bool Position::is_on_seventh_rank(Square s, Color c) const {
    return rank_of(s) == (c == WHITE ? RANK_7 : RANK_2);
}
